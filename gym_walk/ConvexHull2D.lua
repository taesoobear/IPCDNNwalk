...
--class 'ConvexHull2D'
--same as the ConvexHull2D class in taesooLib + a few functions in testTerrainConvexHull.lua
...
function buildHullZY(tpos)
	...
	for i=0, sampling  do
		zmpHull:addPoint(tpos(i).z, tpos(i).y)
	end
	zmpHull:buildHull()
	if zmpHull.supportPlanes:size() >  3 then
		...
		local function search_vtx(v2, tpos)
			for  i=0, tpos:size()-1 do
				if v2.x==tpos(i).z and v2.y==tpos(i).y then
					return i
				end
			end
			assert(false)
...
function projectConvexHull(arr_tpos,  res)
	...
	local function findLines(z) 
		...
		for i=1, #spprt_vtx_idx-1 do
			...
			local z1=math.min(vtx_orig(s1).z, vtx_orig(s2).z)
			local z2=math.max(vtx_orig(s1).z, vtx_orig(s2).z)
	...
	local z_min=vtx_orig:z():minimum()
	local z_max=vtx_orig:z():maximum()

	for i=0, arr_tpos:size()-1 do
		local z=arr_tpos(i).z
		local l=findLines(z)
		if #l==0 then
		...
		for ii, v in ipairs(l) do
			...
			local z1=vtx_orig(s1).z
			local z2=vtx_orig(s2).z
...
function drawConvexHull( res, name, scalef)
...
function avoidObstacle(traj, isL, spprtPos, pendRotY)
	local A=traj:row(0):toVector3()
	local B=traj:row(traj:rows()-1):toVector3()
	...
	-- modifies a, b!
	local function projDist(a, b)
		a.y=0
		b.y=0
		return a:distance(b)
	end

	for i=0, traj:rows()-1 do
		local d=projDist(traj:row(i):toVector3(), spprtPos)
		if d<min_dist then
			min_dist=d
			arg_min=i
		end
	...
	if arg_min then
		...
		if arg_min==0 then arg_min=1 end
		if arg_min==traj:rows()-1 then arg_min=traj:rows()-2 end

		local midf=arg_min


		local swingPos=traj:row(arg_min):toVector3()

		local dot=(swingPos-spprtPos):dotProduct(pendRotY*vector3(1,0,0))
		local sep=0.05
		local err
		if isL and -dot<sep then
			err=-1*(pendRotY*vector3(sep+dot,0,0))
		elseif (not isL) and dot<sep then
			err=pendRotY*vector3(sep-dot, 0,0)
		end
		if err then
			err.y=0
			local origtraj=traj:copy()
			-- generate timewarping curve (길이가 늘어나는 부분의 시간도 부드럽게 늘려주기 위함)
			local A=traj:row(0):toVector3()
			local B=traj:row(traj:rows()-1):toVector3()
			local M=swingPos

			local d1=projDist(A,M)
			local d2=projDist(M,B)

			local fracOrig=d1/(d1+d2)

			--d1=projDist(A, swingPos+err)
			--d2=projDist(swingPos+err, B)
			d1=projDist(A, spprtPos)
			d2=projDist(spprtPos, B)
			local fracNew=d1/(d1+d2)

			RE.output2("frac", midf, fracOrig, fracNew)

			local scaleFactorPart1=fracNew/fracOrig

			local timeFactorOrig=arg_min/(traj:rows()-1)
			local timeFactorNew1=timeFactorOrig*scaleFactorPart1
			local timeFactorNew2=1- (1-timeFactorOrig)*((1-fracNew)/(1-fracOrig))
			
			local timeFactorNew=timeFactorNew1*0.5+timeFactorNew2*0.5

			if not(timeFactorNew>0 and timeFactorNew<1) then
				timeFactorNew=timeFactorNew1
			end
			if not(timeFactorNew>0 and timeFactorNew<1) then
				timeFactorNew=timeFactorNew2
			end
			assert(timeFactorNew>0 and timeFactorNew<1) 


			local sl=math.NonuniformSpline(CT.vec(0, timeFactorNew, 1), 
			CT.vec(0, arg_min, traj:rows()-1):column(), math.NonuniformSpline.ZERO_ACC)
			local points=matrixn()
			local t=vectorn()
			t:linspace(0, 1, traj:rows())
			local timing=matrixn()
			sl:getCurve(t, timing)


			local function sampleVec(mat, i)
				if i<0 then i=0 end
				if i>mat:rows()-1 then i=mat:rows()-1 end
				local v=vectorn()
				mat:sampleRow(i, v)
				return v:toVector3()
			end
			local midfn=timeFactorNew*(traj:rows()-1)
			local nmidfn=math.round(midfn)

			-- part1
			for i=0, nmidfn-1 do
				local s=sop.smoothMapB(i, 0, midfn, 0,1)
				traj:row(i):setVec3(0, sampleVec(origtraj, timing(i,0))+err*s)
			end

			-- part2
			local nf=traj:rows()
			for i=nmidfn, nf-1 do
				local s=sop.smoothMapA(i, midfn, nf-1, 1,0)
				traj:row(i):setVec3(0, sampleVec(origtraj, timing(i,0))+err*s)
			end
			return origtraj
		end
...
function applyDiffTo(traj, newtraj, mod)
	if traj then
		local delta=newtraj-traj
		mod:assign(mod+delta)
	end
end
function clearGroundTraj(gfd, icon, si, ei)
	if ei-si<2 or not g_terrain then
		-- no swing time
		return
	end
	local tpos=vector3N(gfd:sub(si,ei,icon*3, icon*3+3))
	local p1=tpos(0)
	local p2=tpos(tpos:size()-1)
	if p1:distance(p2)<1e-2 then
		-- no swing
		return 
	end
	...
	for i=0, tpos:size()-1 do
		local p=tpos(i)
		local h=g_terrain:_getHeight(p)
		p.y=h+0.05
	end
	...
	front.y=0
	front:normalize()
	tf.rotation:setAxisRotation( vector3(0,1,0), front, vector3(0,0,1))
	tf.translation:zero()

	local function xform(tf, vec)
		local out=vector3N(vec:size())
		for i=0, vec:size()-1 do
			out(i):assign(tf*vec(i))
		end
		return out
	end
	local proj=xform(tf,tpos)
	local hull=buildHullZY(proj)
	if not hull then 
		-- flat ground
		for i=si, ei-1 do
			local data=gfd:row(i)
			local p=data:toVector3(icon*3)
			if p.y<tpos(i-si).y then
				p.y=tpos(i-si).y
				data:setVec3(icon*3, p)
			end
		end
		return 
	end
	projectConvexHull(proj, hull)

	proj=xform(tf:inverse(),proj)
	if debugDraw then
		dbg.namedDraw('Traj',(proj):matView()*100, 'currlineR', 'solidred', 8, 'BillboardLineList')
	end

	for i=si, ei-1 do
		local data=gfd:row(i)
		local p=data:toVector3(icon*3)
		if p.y<proj(i-si).y then
			p.y=proj(i-si).y
			data:setVec3(icon*3, p)
		end
	end
end
function clearGround(data, icon)
	if g_terrain then
		local p=data:toVector3(icon*3)
		local h=g_terrain:_getHeight(p)
		if p.y<h+0.05 then
