...
function ctor(no_replan)
	...
	if useNN then
		if not os.isFileExist('feature_data_info.dat') then
			g_dataset=util.loadTable('feature_data.dat')
			local info={ dataInfo=g_dataset.dataInfo}
			util.saveTable(info,'feature_data_info.dat')
		end
		g_dataset=util.loadTable('feature_data_info.dat')
		python.F('test_walk2', 'loadNN', 'walkrun.net', useDotMapping) -- test_walk2.py. loads walkrun.net
	else
		g_dataset=util.loadTable('feature_data.dat')
...
function PendPlanner:replan()
	...
	if g_FA then
		...
		python.F('test_walk2', 'mapping', feature, vdata)	
		if useDotMapping then
			python.F('test_walk2', 'dotmapping', feature, vdotdata)	
		end
	end
