...
struct RBDL_DLLAPI SpatialTransform {
   ...
   */
inline  SpatialVector apply (const SpatialVector &v_sp) {
    Vector3d v_rxw (
   ...
   */
inline  SpatialVector applyTranspose (const SpatialVector &f_sp) {
    Vector3d E_T_f (
  ...
  /** Same as X^* I X^{-1}
  */
inline  SpatialRigidBodyInertia apply (const SpatialRigidBodyInertia &rbi) {
    return SpatialRigidBodyInertia (
        rbi.m,
        E * (rbi.h - rbi.m * r),
        E * 
  ...
  /** Same as X^T I X
  */
inline  SpatialRigidBodyInertia applyTranspose (const SpatialRigidBodyInertia &rbi) {
...
inline  SpatialVector applyAdjoint (const SpatialVector &f_sp) {
  ...
  // == liegroup.Ad( inverse( SE3(E.T, r) ) ) (except that v and w are swapped)
inline  SpatialMatrix toMatrix () const {
    Matrix3d _Erx =
      E * Matrix3d (
          0., -r[2], r[1],
          r[2], 0., -r[0],
  ...
  // == liegroup.dAd( SE3(E.T, r) ) 
inline  SpatialMatrix toMatrixAdjoint () const {
    Matrix3d _Erx =
      E * Matrix3d (
          0., -r[2], r[1],
          r[2], 0., -r[0],
...
inline  SpatialMatrix toMatrixTranspose () const {
    Matrix3d _Erx =
      E * Matrix3d (
          0., -r[2], r[1],
          r[2], 0., -r[0],
...
inline  SpatialTransform operator* (const SpatialTransform &XT) const {
    return SpatialTransform (E * XT.E, XT.r + XT.E.transpose() * r);
  }

inline  void operator*= (const SpatialTransform &XT) {
