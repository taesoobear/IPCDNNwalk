...
{ 
	...
	return double(_tend.QuadPart-_tstart.QuadPart)/double(_tfreq.QuadPart);
#else
...
void SVD33(double *M, double *S)
{
	double m, detC, q, p, phi, co, si, sqrt_p, sqrt_3, tmp, eps = _EPS;
	double A[9], C[9], E[3];
	...
	q = 0.5*detC;
	p = (C[0]*C[0] + C[1]*C[1] + C[2]*C[2] + C[3]*C[3] + C[4]*C[4] + C[5]*C[5] + C[6]*C[6] + C[7]*C[7] + C[8]*C[8])/6.;
	if ( p == 0. && q == 0. ) { //if ( fabs(p) < eps && fabs(q) < eps ) {
		E[0] = m; E[1] = m; E[2] = m;
	} else {
		if ( q == 0. ) {
			phi = 3.141592653589793/6.;
		} else {
			phi = atan(sqrt(fabs(p*p*p-q*q))/q) / 3.;	// in theroy p^3-q^2>=0, but make sure it by using fabs()
		}
		co = cos(phi);
		si = sin(phi);
		sqrt_p = sqrt(p);
...
void SVD33(const RMatrix &M, RMatrix &U, RMatrix &S, RMatrix &V, double tol)
{
#ifndef RMATRIX3_DO_NOT_CHECK_SIZE
		if ( M.row != 3 || M.col != 3 ) std::cerr << "size is not compatible" << std::endl;
#endif	
...
void SVD33(double *M, double *U, double *S, double *V, double tol)
{
	int idx, rnk;
	double m, detC, q, p, phi, co, si, sqrt_p, sqrt_3, tmp, detM, tmpVec[3], eps = _EPS;
	double A[9], C[9], E[3], pinvS[3];
	...
	// U = M*V*pInv(Diag(S))
	if ( fabs(S[0]) < eps ) { pinvS[0] = 0.; } else { pinvS[0] = 1./S[0]; }
	if ( fabs(S[1]) < eps ) { pinvS[1] = 0.; } else { pinvS[1] = 1./S[1]; }
	if ( fabs(S[2]) < eps ) { pinvS[2] = 0.; } else { pinvS[2] = 1./S[2]; }
	switch (rnk) {
		case 0:
			U[0] = 1.; U[1] = 0.; U[2] = 0.; U[3] = 0.; U[4] = 1.; U[5] = 0.; U[6] = 0.; U[7] = 0.; U[8] = 1.;
			break;
...
bool QRSolveAxEqualB(const RMatrix &A, RMatrix &x, const RMatrix &B)
{
	...
	if ( _work_QR.row < max(p,n) ) _work_QR.ReNew(max(p,n));
...
RMatrix pInv(const RMatrix &A, RMatrix &N, double eps)
{
	int i, cnt;
	RMatrix re, U, s, V, is;
	SVD(A, U, s, V);
...
RMatrix srInv(const RMatrix &A, RMatrix &N, double alpha, double eps)
{
...
RMatrix srInv2(const RMatrix &A, double alpha)
{
	if ( A.row <= A.col ) {
...
void solve_Ax_b_pInv(RMatrix &x, const RMatrix &A, const RMatrix &b, double eps)
{
	RMatrix U, s, V, is;
	SVD(A, U, s, V);
	double mx = MaxVec(s);
...
void solve_Ax_b_pInv(RMatrix &x, const RMatrix &A, const RMatrix &b, RMatrix &N, double eps)
{
	int i, cnt;
	RMatrix U, s, V, is;
	SVD(A, U, s, V);
...
void solve_Ax_b_srInv(RMatrix &x, const RMatrix &A, const RMatrix &b, RMatrix &N, double alpha, double eps)
{
...
void solve_Ax_b_pInvW(RMatrix &x, const RMatrix &A, const RMatrix &b, const RMatrix &iw, double eps)
{
...
void solve_Ax_b_pInvW(RMatrix &x, const RMatrix &A, const RMatrix &b, const RMatrix &iw, RMatrix &Nw, double eps)
{
	int i, cnt;
...
void solve_Ax_b_srInvW(RMatrix &x, const RMatrix &A, const RMatrix &b, const RMatrix &iw, RMatrix &Nw, double alpha, double eps)
{
