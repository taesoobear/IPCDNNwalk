...
public:
	...
	vector3 GetTriple( long i, long j) const;
	...
	void SetTriple( long i, long c, const vector3& u );
...
class MatrixRmnView : public MatrixRmn
{
	public:
		MatrixRmnView(matrixn & J):MatrixRmn()
	{
		x=&J[0][0];
		NumRows=J.cols();
		NumCols=J.rows();
		...
		~MatrixRmnView() { 
			x=NULL; // so that it is not deleted.
		}
...
inline vector3 MatrixRmn::GetTriple( long i, long j) const
{
	vector3 u;
	long ii = 3*i;
	vector3_dump(x+j*NumRows + ii, u);
	return u;
}

// Set the i-th triple in the j-th column to u's three values
inline void MatrixRmn::SetTriple( long i, long j, const vector3& u ) 
{
	long ii = 3*i;
	assert ( 0<=i && ii+2<NumRows && 0<=j && j<NumCols );
	vector3_dump(u, x+j*NumRows + ii );
}
