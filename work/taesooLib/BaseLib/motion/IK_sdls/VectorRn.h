...
public:
	...
	void SetTriple( long i, const vector3& u );
	vector3 toVector3(long i) const { return vector3(x[i], x[i+1], x[i+2]);}
	vector3 GetTriple(long i) const { return vector3(x[i*3], x[i*3+1], x[i*3+2]);}
...
inline 	void vector3_dump( double* v, vector3 & vv ) 
{
	vv.x=*v      ; 
	vv.y=*(v+1)  ;
	vv.z=*(v+2)  ;
...
inline 	void vector3_dump( vector3 const& vv, double* v ) 
{
	*v = vv.x; 
	*(v+1) = vv.y;
	*(v+2) = vv.z;
...
inline void VectorRn::SetTriple( long i, const vector3& u ) 
{
	long j = 3*i;
