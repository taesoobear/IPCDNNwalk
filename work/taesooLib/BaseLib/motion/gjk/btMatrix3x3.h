...
class btMatrix3x3 {
	public:
		...
		btMatrix3x3 (const quater& q) {setRotation( q) ;}
//		explicit btMatrix3x3(const btScalar *m) { setFromOpenGLSubMatrix(m); }
		...
		*/
		...
		SIMD_FORCE_INLINE btScalar& operator()(const std::size_t i,const std::size_t j)
		{
			btFullAssert(0 <= i && i < 3);
			btFullAssert(0 <= j && j < 3);
			if(j==0) return m_el[i].m_x;	
			else if(j==1) return m_el[i].m_y;	
			else if(j==2) return m_el[i].m_z;
		}

		SIMD_FORCE_INLINE btScalar operator()(const std::size_t i,const std::size_t j) const
		{
			btFullAssert(0 <= i && i < 3);
			btFullAssert(0 <= j && j < 3);
			if(j==0) return m_el[i].m_x;	
	...
	void setFromOpenGLSubMatrix(const btScalar *m)
		...
		void getRotation(quater& q) const
		{
