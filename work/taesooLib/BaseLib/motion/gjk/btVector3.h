...
public:
	...
	SIMD_FORCE_INLINE btVector3(const btScalar& all) 
		:btQuadWord(all,all,all,btScalar(0.))
	{
	}
...
//	}
	...
	SIMD_FORCE_INLINE btScalar operator() (const std::size_t i) const{
	//return (&m_floats[0])[i];
	if(i==0) return m_x;
	...
	SIMD_FORCE_INLINE btScalar& operator() (const std::size_t i) {
	if(i==0) return m_x;
	else if(i==1) return m_y;
	else if(i==2) return m_z;
	}
