...
function IPC3d:__step(ntimes,ef, ef_height)
	...
	if false then -- limit speed for numerical stability
	...
	local ef=ef:copy()
	ef:rotate(toLocal)
	-- 폴의 height 위치에 힘이 간다고 가정하면,
	-- torque= position cross force
	-- = (0,1,0) cross ef
	local torque=vector3()
	torque:cross(vector3(0,ef_height,0), ef)

	self.pendX:oneStep(CT.vec(ef.x, -torque.z)) -- current code : force, torque -.-
	self.pendZ:oneStep(CT.vec(ef.z, torque.x))
	--##testmw   if g_debugOneStep then
...
-- ef : external force (vector3) in global
-- ef_height : 작용점 높이.
function IPC3d:oneStep_ef(ef, ef_height)

	self:__step(1,ef, ef_height)
...
function IPC3d:setState_ori_conservative(zmp_pos, com_pos, zmp_vel, com_vel)
