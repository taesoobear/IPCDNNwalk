...
function LQR_wrap(A,B,Q,R)
   ...
   if CLapack2 then
	   -- real lqr
	   CLapack2.lqr(k,A,B,Q,R) -- cpp implementation ported from lua implementation
   else
	   -- uses saved matrix
	   Physics.LQR(k,A,B,Q,R) -- cpp implementation ported from lua implementation
   end
...
function NonlinearController:oneStep(x,U,dt,maxForce,Q,R,K, xd, tau)
   local dim=self.dim
   self.cf=self:calcControlForce(x,U,Q,R,K,xd)
   local cf=self.cf
   return self:oneStepRaw(x,cf,dt,maxForce,Q,R,K, xd, tau)
end
function NonlinearController:oneStepRaw(x,cf,dt,maxForce,Q,R,K, xd, tau)
...
function IPCview:oneStep(tau)


	self.ddtheta= self.IPC:oneStep(self.x, self.U, self.dt, self.maxForce,self.Q, self.R, self.K, self.xd, tau)
   self.t=self.t+self.dt
...
function DIPCview:setQ(q)
   ...
   self.Q:diag():setValues(0,q,q,q,0,0)
...
function DIPCview:oneStepNoControl()
	local cf=self.cf

	local dim=self.DIPC.dim
   self.DIPC:updateSDRE(self.x:column(0):range(0,dim), self.x:column(0):range(dim, dim*2))
   self.DIPC:oneStepRaw(self.x,self.U,self.dt,self.maxForce,self.Q, self.R, self.K, self.xd, self.tau)
   self.t=self.t+self.dt
end
