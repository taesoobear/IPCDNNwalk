# genesis/engine/scene.py가져와서 visualization 떼버림. taesooLib쓸껀데 어차피 필요 없고, 쓸데없이 생성이 오래걸려서. 자꾸 죽기나 하고...
# 나중에 gs버젼 업데이트 해도 쉽게 업데이트 가능함 (원본 그대로 가져와서 _visualizer사용하는줄 모두 삭제 후 __init__에 self._visualizer=None 추가) gs는 self._visualizer =None 이어도 잘 동작함.
import numpy as np
import torch
...
class Scene(RBC):
    """
    A ``genesis.Scene`` object wraps all components in a simulation environment, including a simulator (containing multiple physics solvers), entities, but no visualizer 
