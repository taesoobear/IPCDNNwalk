...
function SDS:updateCoef(B, k,Q)

	self.b=B
	self.k=k
	assert(self.b)
	assert(self.k)
	assert(type(self.k)=='number')
	local unused=CT.vec(0)
	self:updateSDRE(unused, unused)
	self.dGdTheta_0:set(0, self.k/self.M)
...
MassParticle3D=LUAclass()
function MassParticle3D:__init(mass, B, k, q, qd , dt)
	assert(B)
	self.x=SDS(mass, B, k, q, qd, dt)
	self.y=SDS(mass, B, k, q, qd, dt)
	self.z=SDS(mass, B, k, q, qd, dt)
end
function MassParticle3D:setState(pos, vel)
	if not vel then vel=vector3(0,0,0) end
	self.x.x:set(0,0, pos.x)
	self.y.x:set(0,0, pos.y)
	self.z.x:set(0,0, pos.z)
	self.x.x:set(1,0, vel.x)
	self.y.x:set(1,0, vel.y)
	self.z.x:set(1,0, vel.z)
end
function MassParticle3D:getPosition()
	local out=vector3()
	out.x=self.x.x(0,0)
	out.y=self.y.x(0,0)
	out.z=self.z.x(0,0)
...
function MassParticle3D:getVelocity()
	local out=vector3()
	out.x=self.x.x(1,0)
	out.y=self.y.x(1,0)
	out.z=self.z.x(1,0)
...
function MassParticle3D:setDesiredState(pos, vel)
	if not vel then vel=vector3(0,0,0) end
	self.x.xd:set(0,0, pos.x)
	self.y.xd:set(0,0, pos.y)
	self.z.xd:set(0,0, pos.z)
	self.x.xd:set(1,0, vel.x)
	self.y.xd:set(1,0, vel.y)
	self.z.xd:set(1,0, vel.z)
end
function MassParticle3D:singleStep()
	self.x:singleStep()
	self.y:singleStep()
	self.z:singleStep()
end
function MassParticle3D:updateCoef(B, k, Q)
	self.x:updateCoef(B, k, Q)
	self.y:updateCoef(B, k, Q)
	self.z:updateCoef(B, k, Q)
end
...
function IPCview:__draw(entityname, x)
...
function DIPCview:setQ(q)
	...
	-- cart, ankle, hip, cartvel, anklevel, hipvel
   self.Q:diag():setValues(0,q,0.00001*q,q,0,0)
...
function DIPCview:__init(m0,m1,m2,l1,l2,I1,I2, dt,q)
   ...
   if cdim==2 then
      self.R:diag():setValues(1,10) -- 1,1 doesn't work for running. (
   elseif cdim==3 then
      self.R:diag():setValues(1,1000,1)
   else
...
function DIPCview:oneStep()
	...
	if self.useDesiredPos then
		...
		self.Q:diag():setValues(100*q,q,q,0,0,0)
		self.K=LQR_wrap(self.DIPC.A,self.DIPC.B,self.Q,self.R)
	end
...
function DIPCview:__draw(entityname, x)
	local pNode=RE.createEntity(entityname.."cart_pole", "cube.mesh");
	local pNode2=RE.createEntity(entityname.."upperBody", "cube.mesh");
	local v1=vector3()
	local v2=vector3()
	local v3=vector3()
	local q=quater()
