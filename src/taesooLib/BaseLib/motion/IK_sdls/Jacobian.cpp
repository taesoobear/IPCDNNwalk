...
void Jacobian::SetJendActive() { 
	Jend.SetSize(nRow, nCol);				// The Jocobian matrix
	Jend.SetZero();
	Jtarget.SetSize(0,0); // unused
	Jactive = &Jend;
}						// The default setting is Jend.
void Jacobian::SetJtargetActive() { 

	Jtarget.SetSize(nRow, nCol);			// The Jacobian matrix based on target positions
	Jtarget.SetZero();
	Jend.SetSize(0,0); // unused
	Jactive = &Jtarget; 
}
void Jacobian::Reset() 
{
...
void Jacobian::ComputeJacobian() 
{
	...
	for(int ii=0; ii<tree->effectors.size(); ii++)
	{
		...
		n->computeDeltaS(dS, tree);

		if(useJend)
			n->calcJacobian(tree, Jend, i, n->GetS());
		else
		{
			// target jacobian 
			const vector3& targetPos = tree->target[i];
			n->calcJacobian(tree, Jtarget, i, targetPos);
		}
	}
}
...
void Jacobian::UpdateThetas() 
{
	...
	while ( n ) {	
		if ( n->IsJoint() ) {
			int i = n->GetJointNum();
			((HingeJoint*)n)->AddToTheta( dTheta[i] );
		}
		else if(n->IsFreeJoint()){
			((FreeJoint*)n)->Integrate(dTheta);
		}
...
void Jacobian::UpdatedSClampValue()
{
	...
	for(int ii=0; ii<tree->effectors.size(); ii++)
	{
		n = tree->effectors[ii];
			...
			double changedDist = temp.length()-normSi;
			if ( changedDist>0.0 ) {
