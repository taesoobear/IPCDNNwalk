...
RBDL_DLLAPI void ForwardDynamics (
  ...
  for (i = 1; i < model.mBodies.size(); i++) {
    ...
    model.v[i] = model.X_lambda[i].apply( model.v[lambda]) + model.v_J[i];
       ...
       LOG << "SpatialVelocity (" << i << "): " << (model.v[i]).transpose() << std::endl;
       LOG << "w_v (" << i << "): " << (model.X_base[i].inverse().toMatrix()*model.v[i]).transpose() << std::endl;
#endif
    model.c[i] = model.c_J[i] + crossm(model.v[i],model.v_J[i]);
...
#ifdef RBDL_ENABLE_LOGGING
	LOG << "c: "<<model.c[i]<<std::endl;
	LOG << "w_c: "<<model.X_base[i].toMatrixTranspose()*model.c[i]<<std::endl;
	LOG << "w_IA: "<<model.X_base[i].toMatrixTranspose()*model.IA[i]*model.X_base[i].toMatrix()<<std::endl;
	LOG << "pA: "<<model.pA[i].transpose()<<std::endl;
	LOG << "w_pA: "<<(model.X_base[i].inverse().toMatrixAdjoint()*model.pA[i]).transpose()<<std::endl;
    ...
    if (f_ext != NULL && (*f_ext)[i] != SpatialVector::Zero()) {
	  ...
	  //std::cout << "External force (" << i << ") = " << model.X_base[i].toMatrixAdjoint() * (*f_ext)[i] << std::endl;
	  //std::cout << "External force2 (" << i << ") = " <<  (*f_ext)[i] << std::endl;
...
#ifdef RBDL_ENABLE_LOGGING
	  LOG<<"w_S("<<i<<"):"<<model.X_base[i].inverse().toMatrix()*model.multdof3_S[i]<<std::endl;
	  LOG<<"S("<<i<<"):"<<model.multdof3_S[i]<<std::endl;
	  LOG<<"w_U("<<i<<"):"<<model.X_base[i].inverse().toMatrixAdjoint()*model.multdof3_U[i]<<std::endl;
#endif
...
#ifdef RBDL_ENABLE_LOGGING
       LOG << "Dinv[" << i << "] = " << model.multdof3_Dinv[i] << std::endl;
       LOG << "multdof3_u[" << i << "] = " << model.multdof3_u[i].transpose() << std::endl;
	   LOG <<"Dinv*U"<<model.multdof3_Dinv[i]*model.multdof3_u[i]<<std::endl;
#endif
      unsigned int lambda = model.lambda[i];
      if (lambda != 0) {
        SpatialMatrix Ia = model.IA[i]
