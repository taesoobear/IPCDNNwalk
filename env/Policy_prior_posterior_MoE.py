...
from .convvq import VQEmbeddingMovingAverage
import numpy as np
...
#     def __init__(self, latent_dim_pi, prior_dim, hidden_dim):
...
#
#         # 定义每一层 Linear
#         self.fc1 = nn.Linear(latent_dim_pi + prior_dim, hidden_dim)
#         self.fc2 = nn.Linear(hidden_dim + latent_dim_pi, hidden_dim)
#         self.fc3 = nn.Linear(hidden_dim + latent_dim_pi, hidden_dim)
#
#         # 激活函数
#         self.activation = nn.Tanh()
#
#     def forward(self, prior, latent_pi):
#         """
#         prior: (batch_size, prior_dim)
#         latent_pi: (batch_size, latent_dim_pi)
#         """
#         # 第一层，concat latent_pi 和 prior
#         x = torch.cat([latent_pi, prior], dim=-1)
#         x = self.activation(self.fc1(x))
#
#         # 第二层，concat latent_pi
#         x = torch.cat([latent_pi, x], dim=-1)
#         x = self.activation(self.fc2(x))
#
#         # 第三层，concat latent_pi
#         x = torch.cat([latent_pi, x], dim=-1)
...
class SRBPolicy(ActorCriticPolicy):
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        self.bottle_neck = VQEmbeddingMovingAverage(
            K = 128, # 码表数目/ Code table number
            D = latent_dim_pi, # 每个码的维度/ Dimensions of each code
            training= False # 刚开始不在训练/ Not training at first
        )
        ...
        self.log_std = nn.Parameter(torch.full((self.continuous_dim,), -1.0))  # -0.5
    ...
    def _build_mlp_extractor(self) -> None:
        self.mlp_extractor = CustomNetwork(self.features_dim, hidden_dim=self.hidden_dim)
        self.prior_extractor = CustomNetwork(self.prior_dim, hidden_dim=self.hidden_dim)
        self.categorical_head = nn.Linear(32, 128)

    def train(self, training):
        self.bottle_neck.is_training = training

    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        prior_obs = obs[..., :self.prior_dim]

        features = self.extract_features(obs)
        assert torch.allclose(features, obs)
        latent_pi, latent_vf = self.mlp_extractor(features)
        # latent_vq, latent_vq_bar, indices = self.bottle_neck.straight_through(latent_pi)
        #
        # prior_obs = obs[..., :self.prior_dim]
        #
        # latent_prior, _ = self.prior_extractor(prior_obs)
        # categorical_logits = self.categorical_head(latent_prior)
        # distribution = torch.distributions.Categorical(logits=categorical_logits)
        # indices = distribution.sample()
        # latent_prior = self.bottle_neck.embedding[indices]
        #
        # """
        # Notice:
        # This is very important.
        # -When latent_pi = latent_vq, it's tracking with posterior encoder together
        # -When latent_pi = latent_prior, it's random sampling with prior encoder solely
        # During training, we use latent_pi = latent_vq, when checking the result you could choose whatever you want.
        # """
        # latent_pi = latent_vq
        # # latent_pi = latent_prior
        ...
        # print(delta)
        # print('----------------')
        # w. actual feet state, w./o. mocap feet state
    ...
    def evaluate_actions(self, obs: torch.Tensor, actions: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # latent_vq, latent_vq_bar, indices = self.bottle_neck.straight_through(latent_pi)
        # commit_loss = (torch.norm(latent_pi - latent_vq_bar.detach()) ** 2) * 0.05 / np.prod(latent_pi.shape[:-1])
        # latent_pi = latent_vq
        prior_obs = obs[..., :self.prior_dim]
        #
        # latent_prior, _ = self.prior_extractor(prior_obs)
        ...
        # categorical_logits = self.categorical_head(latent_prior)
        #
        # ce_loss = F.cross_entropy(categorical_logits, indices)
        ...
        next_feet_contact_target = obs[..., -4:]
        ...
        # ce_loss += ((action_foot_xy-obs_foot_xy)**2).mean()*10
        ce_loss = ce_loss+((action_foot_xy - obs_foot_xy) ** 2).mean()*50

        # recularization term
        # regularization_loss = (delta**2).mean()
        # ce_loss = ce_loss+regularization_loss*0.01*weight_ratio  # (0.1+0.1*weight_ratio)

        # print(next_feet_ce_loss)
        # print(((action_foot_xy - obs_foot_xy) ** 2).mean()*50)
        # print(regularization_loss*5)
        # print(distance_loss*5)
        # print('------------------------')
