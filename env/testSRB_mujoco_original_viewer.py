...
def get_feet_state_forward(model, data):
    feet_list = ['left_foot', 'right_foot']
    feet_pos_list = []
    feet_ori_list = []
    for foot in feet_list:
        ...
        temp_foot_pos = data.geom_xpos[foot_id].copy()
        temp_foot_ori = data.geom_xmat[foot_id].reshape(3, 3).copy()
        temp_foot_ori_z = get_z_rotation_matrix_forward(temp_foot_ori)
        feet_pos_list.append(temp_foot_pos)
        feet_ori_list.append(temp_foot_ori_z)
    return feet_pos_list, feet_ori_list
def get_z_rotation_matrix_forward(xmat):
    """
    给定脚的旋转矩阵 xmat（x: forward, z: up）和角色整体朝向 com_x，
    当脚的 x 与其地面投影夹角超过 90°（即脚尖翘起）且脚尖指向后方时，
    使用 z_proj + com_forward 构造更稳定的前向。

    参数:
        xmat: (3, 3) 脚的旋转矩阵
        com_x: (3,) 当前角色的 x 方向（通常为 root 的 x 轴）

    返回:
        z_rot_matrix: (3, 3) 表示绕 z 轴的旋转矩阵
    """
    foot_x = xmat[:, 0].copy()  # 脚的 forward（x 轴）
    foot_y = xmat[:, 1].copy()  # 脚的 left（y 轴）
    ...
    y_proj = foot_y.copy()
    y_proj[2] = 0
    y_proj_norm = np.linalg.norm(y_proj)
    y_proj /= y_proj_norm
    if x_proj_norm < 1e-6:
        # fallback：几乎垂直，投影太短
        use_y = True
    else:
        # 判断脚的 x 与其地面投影的夹角是否 > 90°
        dot_x_proj = np.dot(foot_x, x_proj)
        use_y = dot_x_proj < 0.3
    z = np.array([0, 0, 1])

    x_proj_new = np.cross(y_proj, z)
    x_proj_new /= np.linalg.norm(x_proj_new + 1e-8)
    if use_y:
        x_proj = 0.9*x_proj_new+0.1*x_proj
    else:
        x_proj = 0.7*x_proj_new + 0.3*x_proj
    # 最终在 xy 平面上计算角度
    theta = np.arctan2(x_proj[1], x_proj[0])
...
def set_feet_site_this_step(model, data, feet_pos_list_this_step, feet_ori_list_this_step):  # set feet state in simulation
    ...
    j = 0
    offset = 5
    site_to_apply_force_on = [-1] * len(feet_site_list_SRB)  # 使用 -1 作为占位符
    ...
    rel_lf = np.array([0.15, 0.10, 0])
    rel_rf = np.array([0.15, -0.10, 0])
    rel_lb = np.array([-0.15, 0.10, 0])
    rel_rb = np.array([-0.15, -0.10, 0])


    while j < len(feet_site_list_SRB):
        if feet_pos_list_this_step[i][2] != 0:  # 如果 z 坐标不为 0，跳过该 site
            site_ids_to_hide = [
            ...
            for k, site_id in enumerate(site_ids_to_hide):
                if i == 0:
                    model.site_rgba[site_id] = [1, 0, 0, 0.3]
                else:
                    model.site_rgba[site_id] = [0, 1, 0, 0.3]
                site_to_apply_force_on[j + k] = -1  # 设置为 -1
        ...
        data.site_xpos[site_ids_to_show[0]] = feet_pos_list_this_step[i]
        data.site_xpos[site_ids_to_show[1]] = feet_pos_list_this_step[i] + feet_ori_list_this_step[i] @ rel_lf
        data.site_xpos[site_ids_to_show[2]] = feet_pos_list_this_step[i] + feet_ori_list_this_step[i] @ rel_rf
        data.site_xpos[site_ids_to_show[3]] = feet_pos_list_this_step[i] + feet_ori_list_this_step[i] @ rel_lb
        data.site_xpos[site_ids_to_show[4]] = feet_pos_list_this_step[i] + feet_ori_list_this_step[i] @ rel_rb
        j += offset
        i += 1
...
def apply_force_at_site(model, data, site_id, force):
    ...
    #RE.draw('SphereM',RE.toVector3(site_pos).ZtoY()+RE.toVector3([-1,0,0]),f'forcepos{site_id}','red',0.05)
...
class SRBdata(object):
    def __init__(self, mocap_path):
        self.character_path = "gym_trackSRB/Characters/SRB5.xml"
        self.mocap_path = mocap_path
        self.mocap_contact_pair = {
            'motiondata_mujoco_refined/fight1_subject3.txt': 'ref_contact/fight1_subject3.bvh.constraint.npz',
            'motiondata_mujoco_refined/walk3_subject5.txt': 'ref_contact/walk3_subject5.bvh.constraint.npz',
            'motiondata_mujoco_refined/walk_run_jump.txt': 'ref_contact/walk_run_jump.bvh.constraint.npz',
            'motiondata_mujoco_refined/walk_jump.txt': 'ref_contact/walk_jump.bvh.constraint.npz',
            'motiondata_mujoco_refined/straight_walk_scaled_y_global.txt': 'ref_contact/straight_walk_scaled_y_global.bvh.constraint.npz',
            'motiondata_mujoco_refined/loco_full.txt': 'ref_contact/loco_full.bvh.constraint.npz',
            ...
            'ref_contact/mvae.motion.npz': 'ref_contact/mvae.constraint.npz',
    ...
    def humanoid2SRB(self, initial_frame=0, last_frame=None):
        ...
        while True:
            ...
            temp_feet_pos, temp_feet_ori = get_feet_state_forward(self.model, self.data)
        ...
        cdm_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, 'cdm')
        # cdm_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_GEOM, 'cdm')
        self.T_mocap_com = []
        ...
        for i in range(self.mocap_steps_num):
            ...
            temp_T_mocap = transformation_matrix(data.xmat[cdm_id].copy().reshape(3, 3), data.subtree_com[0])
            # temp_T_mocap = transformation_matrix(data.geom_xmat[cdm_id].copy().reshape(3, 3), data.geom_xpos[cdm_id][:3])
            self.T_mocap_com.append(temp_T_mocap)
        ...
        with (mujoco.viewer.launch_passive(model, data, show_left_ui=False, show_right_ui=False, key_callback=key_callback) as viewer):
            ...
            while i <= self.mocap_steps_num-2:
                if not paused:
                    ...
                    curr_T_mocap = self.T_mocap_com[i+1]
                    ...
                    for _ in range(round(self.dt/model.opt.timestep)):
                        ...
                        site_to_apply_force_on = set_feet_site_this_step(model, data, curr_feet_pos, curr_feet_ori)
...
if __name__ == "__main__":
    ...
    def key_callback(keycode):
        ...
        if chr(keycode) == ' ':
            paused = not paused
    mocap_path = "motiondata_mujoco_refined/jumps1_subject1.txt"
